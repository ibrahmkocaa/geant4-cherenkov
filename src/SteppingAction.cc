...
void SteppingAction::UserSteppingAction(const G4Step *step) {
  ...
  G4cout << "[Event " << eventID << "] [Track " << trackID << "] [Step "
         << stepNumber << "] [Neutron] "
         << "Proc=" << processName;

  // Hedef çekirdek bilgisi (Z, A) (tüm hadronic süreçler için)
  if (process->GetProcessType() == fHadronic) {
    G4HadronicProcess *hadProc =
        dynamic_cast<G4HadronicProcess *>(const_cast<G4VProcess *>(process));
    if (hadProc) {
  ...
  G4cout << " | Pos=(" << position.x() / mm << ", " << position.y() / mm << ", "
         << position.z() / mm << ") mm"
  ...
  if (secondaries && !secondaries->empty()) {
    ...
    for (const auto &sec : *secondaries) {
      ...
      G4cout << "\n       * " << secName << " | Ekin=" << secEkin / MeV
             << " MeV"
             << " | Pos=(" << secPos.x() / mm << ", " << secPos.y() / mm << ", "
             << secPos.z() / mm << ") mm";
    }
  }
  ...
  if (status == fStopAndKill && processName == "Transportation") {
    ...
    G4cout << " | ExitPos=(" << position.x() / mm << ", " << position.y() / mm
           << ", " << position.z() / mm << ") mm";
  }
